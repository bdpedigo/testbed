...
def vertices_on_chunk_boundaries(vertices, bbox, cv):
    ...
    """
    draco_grid_size = cv.meta.get_draco_grid_size(0)
    tol = draco_grid_size  # max displacement from draco quantization
...
def _triangulate_face_polygon(polygon_3d, axis):
    ...
    # Remove near-duplicate consecutive vertices (e.g. from bbox snapping)
    ...
    # if len(dup_pairs) > 0:
    #     # Jitter duplicates slightly to un-pinch the polygon
    ...
    # Remap indices back to original polygon_3d positions
    return keep[tri_output["triangles"]]


def cap_mesh_at_bbox(mesh, bbox, cv):
...
def fill_holes(mesh, hole_size=10000):
    poly = mesh_to_poly(mesh)
    filled = poly.fill_holes(hole_size)  # Large max hole size to fill all holes
...
def fix_mesh(mesh):
    ...
    # mf.repair(remove_smallest_components=False, joincomp=False)
    mf.clean()
...
def remove_degenerate_faces(mesh):
    ...
    These cause non-manifold edges that break boundary loop tracing.
    Unlike pymeshfix.clean(), this won't delete small open shells.
    """
    verts, faces = mesh
    degenerate = (
        (faces[:, 0] == faces[:, 1])
...
def is_valid_mesh(mesh):
    ...
    Returns False for flat meshes (zero extent in any dimension)
    or meshes with fewer than 4 faces (can't form a closed surface).
    """
    verts, faces = mesh
    if faces.shape[0] < 4:
        return False
...
for i, node_id in tqdm(enumerate(node_ids[:]), total=len(node_ids)):
    ...
    mesh = meshes[node_id]
    print(
        f"Processing {node_id} ({mesh[0].shape[0]} verts, {mesh[1].shape[0]} faces)",
        flush=True,
    )
    ...
    mesh = cap_mesh_at_bbox(mesh, bbox, cv)
    ...
    mesh_is_watertight = is_watertight(mesh)

    if not mesh_is_watertight:
        print("Node ID:", node_id)
        print(f"Watertight: {mesh_is_watertight}")
        plotter = pv.Plotter()
        ...
        nonmanifold = pv.make_tri_mesh(*mesh).extract_feature_edges(
            boundary_edges=False,
            non_manifold_edges=True,
            feature_edges=False,
            manifold_edges=False,
        )
        # plotter.add_mesh(nonmanifold, color="red", line_width=5)
        centroid = mesh[0].mean(axis=0)
...
# %%
# problem nodes for future testing: 377805423803150496, 378372771803079313, 374964285757002229,
# node_id = 374964285757002229
...
# plot planar holes in red
plotter = pv.Plotter()
plotter.add_mesh(pv.make_tri_mesh(*mesh), color="lightgray")

all_planar_loop_vertices = set()
loop_info = find_planar_boundary_loops(mesh[0], mesh[1], bbox)
for res in loop_info:
    # if res["spread"] > 100:
    loop_pts = mesh[0][res["loop_indices"]]
    all_planar_loop_vertices.update(res["loop_indices"])
    plotter.add_points(loop_pts, color="red", point_size=10)


# %%
# plot the cloudvolume version, is_draco_chunk_aligned()
...
is_boundary = is_draco_chunk_aligned(mesh[0] - offset, chunk_size, draco_grid_size)
vertices_on_boundaries = np.flatnonzero(is_boundary)

# box = pv.Box(bounds=bbox.T.flatten().tolist())
# plotter = pv.Plotter()
# plotter.add_mesh(pv.make_tri_mesh(*mesh), color="lightgray")
# plotter.add_mesh(box, color="black", opacity=1, style="wireframe")
# plotter.add_points(mesh[0][vertices_on_boundaries], color="red", point_size=10)
# plotter.enable_fly_to_right_click()
# plotter.show()

bbox_raw = node_bbox(node_id, cv, adjust_draco=False)

# All chunk boundaries (internal + external), tight cv tolerance
is_any_boundary = is_draco_chunk_aligned(mesh[0] - offset, chunk_size, draco_grid_size)

# Outer bbox faces only, same tight tolerance
tol = draco_grid_size
is_on_outer_face = np.zeros(len(mesh[0]), dtype=bool)
for axis in range(3):
    is_on_outer_face |= np.abs(mesh[0][:, axis] - bbox_raw[0, axis]) < tol
    is_on_outer_face |= np.abs(mesh[0][:, axis] - bbox_raw[1, axis]) < tol

# Separate categories
is_outer_boundary = is_on_outer_face
is_internal_boundary = is_any_boundary & ~is_on_outer_face

vertices_on_any_boundary = np.flatnonzero(is_outer_boundary | is_internal_boundary)

# Plot outer (red) vs internal (blue) boundary vertices
box = pv.Box(bounds=bbox_raw.T.flatten().tolist())
plotter = pv.Plotter()
plotter.add_mesh(pv.make_tri_mesh(*mesh), color="lightgray")
plotter.add_mesh(box, color="black", opacity=1, style="wireframe")
if np.any(is_outer_boundary):
    plotter.add_points(mesh[0][is_outer_boundary], color="red", point_size=10)
if np.any(is_internal_boundary):
    plotter.add_points(mesh[0][is_internal_boundary], color="blue", point_size=10)
plotter.enable_fly_to_right_click()
plotter.show()

# %%
# check that all planar loop vertices are on a chunk boundary
loop_vertices = set()
for res in loop_info:
    loop_vertices.update(res["loop_indices"])
loop_vertices = np.array(list(loop_vertices))

loop_on_boundary = np.isin(loop_vertices, vertices_on_any_boundary)
loop_on_boundary
# %%

# plot loop vertices, coloring by whether they are on a boundary
box = pv.Box(bounds=bbox_raw.T.flatten().tolist())
plotter = pv.Plotter()
plotter.add_mesh(pv.make_tri_mesh(*mesh), color="lightgray")
plotter.add_mesh(box, color="black", opacity=1, style="wireframe")
plotter.add_points(mesh[0][loop_vertices[loop_on_boundary]], color="red", point_size=10)
plotter.add_points(
    mesh[0][loop_vertices[~loop_on_boundary]], color="blue", point_size=10
)
plotter.add_points(mesh[0][vertices_on_boundaries], color="green", point_size=5)
plotter.enable_fly_to_right_click()
plotter.show()
...
# plot vertices on ANY chunk boundary AND a planar loop in red
bbox = node_bbox(node_id, cv)
loop_info = find_planar_boundary_loops(mesh[0], mesh[1], bbox)
loop_vertices = set()
for res in loop_info:
    loop_vertices.update(res["loop_indices"])
highlight_verts = np.array(
    list(set(vertices_on_boundaries).intersection(loop_vertices))
)
box = pv.Box(bounds=bbox.T.flatten().tolist())
plotter = pv.Plotter()
plotter.add_mesh(pv.make_tri_mesh(*mesh), color="lightgray")
plotter.add_mesh(box, color="black", opacity=1, style="wireframe")
plotter.add_points(mesh[0][is_any_boundary], color="red", point_size=10)
plotter.enable_fly_to_right_click()
plotter.show()
...
plotter.add_mesh(box, color="black", opacity=1, style="wireframe")
plotter.add_points(mesh[0][vertices_on_any_boundary], color="red", point_size=10)
plotter.enable_fly_to_right_click()
plotter.show()
